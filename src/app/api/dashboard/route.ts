...
export async function GET() {
  ...
  for (const item of cellarItems) {
    typeCount[item.wine.type] = (typeCount[item.wine.type] ?? 0) + item.quantity;
  }
  ...
  const drinkSoon = cellarItems
    .filter((item) => item.drinkBefore && new Date(item.drinkBefore) <= sixMonths)
    .map((item) => {
      const drinkBefore = new Date(item.drinkBefore!);
      let status: "urgent" | "optimal" | "soon" = "soon";
      if (drinkBefore < now) status = "urgent";
      else if (drinkBefore < new Date(now.getTime() + 2 * 30 * 24 * 60 * 60 * 1000))
        status = "optimal";
    ...
    .sort((a, b) => new Date(a.drinkBefore!).getTime() - new Date(b.drinkBefore!).getTime());
